...
import BGimage from '../images/spider.webp'; // Background image
...
function Email_sendPage() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const response = await axios.get(`${ENV_File.backendURL}/team/teamss/${tournamentid}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${ENV_File.backendURL}/email`, { subject, message, email });
  ...
  style={{
    backgroundImage: `url(${BGimage}?v=1)`, // Background image
  }}
>
  ...
  {/* Main container */}
  <div className="backdrop-blur-sm bg-black/20 border-2 border-amber-400/30 p-8 sm:px-4 rounded-lg w-full max-w-6xl shadow-lg grid grid-cols-1 lg:grid-cols-2 gap-8">
    {/* Form Section */}
    <div>
